...
- **Name:** Tanauan Medical Center — Public Website (v1)
- **What it is:** A one-stop-shop website for Tanauan Medical Center and its affiliated facilities (C.P. Reyes Satellite Clinic, Lab To Go Medical & Diagnostic Center, Malvar Kidney & Dialysis Center, Tanauan Kidney & Dialysis Center). v1 is the public informational site; later phases add patient and employee portals per Hospital-One-Stop-Shop-Website-Plan.md.
- **Who uses it:** v1 — the general public / prospective and existing patients, unauthenticated. Later phases add authenticated patients and hospital employees.
...
<!-- Versions below are targets approved in TASK A; TASK C installs and overwrites with the
     real installed versions read back from the lockfile. -->

| Layer | Choice | Version | Why (one line) |
|---|---|---|---|
| Runtime / language | Node.js + TypeScript | Node v24.18.0 / TS 5.9.3 | Type safety, standard for Next.js |
| Framework | Next.js (App Router) | 15.5.25 | Boring, well-documented, SSR/SEO for public content. Bumped from originally-approved 14.x after TASK C found unpatched critical/high CVEs (incl. unauthenticated RCE on Windows hosts) in the latest available 14.2.35 — Sonny approved moving to 15 on 2026-09-10 |
| Build tool | Next.js built-in | bundled | Ships with the framework |
...
| Auth | none | — | No login anywhere in v1 |
| Tests | Jest + React Testing Library | TBD in TASK E | Standard, matches plan docs |
| Lint / format | ESLint + Prettier | ESLint 8.57.1 / Prettier TBD in TASK D | Standard Next.js tooling |
| Deployment host | Vercel | — | Native host for Next.js, per plan docs |

**Known accepted residual risk:** `npm audit` still flags a high-severity PostCSS advisory (XSS/path-traversal via `sourceMappingURL`) bundled *inside* `next/node_modules/postcss`. The only fix is Next.js 16 (a further breaking major). Accepted for now because the vulnerable code path runs at build time against source the project itself controls, not against untrusted input at request time on the deployed site — re-evaluate when Next 16 stabilizes further or if this project ever processes untrusted CSS/source maps.

**Deferred, not banned forever — revisit when the phase that needs it actually starts:**
Zustand, React Query, next-auth, Sentry, next-intl, axios, recharts, react-calendar, jwt-decode,
any backend framework, any database, any payment or HMO/insurance integration, n8n.
...
TMC/
├── .gitignore
├── CLAUDE.md / TASKS.md / LESSONS.md   # control files
├── README.md
├── *.md                                 # planning docs (not shipped code)
├── Photo references/                    # source images, not the Next.js public/ folder
├── icons src/                           # Heroicons reference material
└── (app code arrives in TASK C — create-next-app scaffolds src/app, src/components, public/)
```

- **File naming:** components `PascalCase.tsx`, everything else `kebab-case` or `camelCase.ts` per Next.js convention.
- **Where each kind of code goes:** pages/routing → `src/app/`; presentation → `src/components/{layout,shared,public}`; pure helpers/constants → `src/lib/`. No `services/`, `store/`, or `hooks/` yet — v1 has no backend calls or client state to justify them.
- **Import style:** alias `@/*` → `src/*` (set in TASK C).
- **Test file location:** beside source, `*.test.tsx`.
...
6. **Verify after every task.** Run:
   ...
   npm run verify
   ```
   Chains, shortest-feedback-first: `format:check` → `lint` (`--max-warnings=0`) → `test` → `build`. Confirmed green in TASK F (2026-09-10).
...
- **Formatter:** Prettier 3.9.6 (`.prettierrc.json`) — decides all formatting. Do not hand-format. Run `npm run format`.
- **Quotes / semicolons / line width:** formatter decides (single quotes, semicolons, 100 cols — see `.prettierrc.json`).
- **Linter:** ESLint 8.57.1 via `next lint --max-warnings=0` (`npm run lint`) — `next/core-web-vitals`, `next/typescript`, `prettier` (disables stylistic rules Prettier already owns). `--max-warnings=0` is load-bearing: without it, `next lint` exits 0 even with warnings present.
- **Export style:** named exports for components and helpers; `export default` only for Next.js page/layout files (required by the framework's routing convention).
