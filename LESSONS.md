...
- [2026-09-11] The browser's own `SpeechSynthesisUtterance` (Web Speech API) is a genuinely free, no-backend way to have the chat assistant "speak" its scripted answers — no API key, no npm package, works within the no-backend v1 stack. Its `onstart`/`onend` events are the right hook for driving a "talking" avatar animation (play video on start, freeze/rewind on end) rather than trying to time the animation to the text length manually. Always `speechSynthesis.cancel()` any prior utterance before starting a new one (rapid-fire questions otherwise queue and overlap oddly) and on component unmount (so closing the chat panel doesn't leave the assistant talking to an empty screen).
- [2026-09-11] When the same canvas-keying algorithm is needed by two components (the floating launcher's video and the chat panel's talking avatar), extract just the pure pixel-manipulation function into `src/lib/` rather than the whole component — the two callers still differ in real ways (one needs an unmuted `.play()` called synchronously inside a user-gesture handler to satisfy browser autoplay-with-sound policy, the other is muted and purely state-driven) that don't force-fit into one shared component without adding risk to the already-working gesture-timing-sensitive one.
- [2026-09-11] Don't trust `speechSynthesis`'s default voice to sound a particular gender — it's whatever the browser/OS ships as its first/default entry (Chrome and Edge on the same Windows machine picked differently in testing here), and `speechSynthesis.getVoices()` can return an empty array on the very first call (voices load asynchronously; listen for the `voiceschanged` event too). If a specific voice character matters, pick one explicitly by name from a priority list and set `utterance.voice`, rather than relying on the platform default.
- [2026-09-11] Real Chrome/Edge only grant "user activation" (required to `.play()` an unmuted `<video>`) on an actual interaction like a click or keydown — a plain `mouseenter`/hover does NOT count, even though it's a real mouse movement. Result: the floating chat launcher's unmuted hover-to-play worked on every hover *after* the first click anywhere on the page, but silently failed (promise rejected, caught, nothing visible) on a fresh page's very first hover — exactly the "works after I click once" symptom Sonny reported. This is a genuine platform restriction with no JS workaround (it exists specifically to stop unsolicited audio) — tried a muted-fallback-on-rejection first, but Sonny preferred accepting the limitation outright over a hover that's ever silently muted, so the fix was reverted back to a plain unmuted `.play().catch(() => {})`: before the page's first real click, hover may not produce sound at all; after that click, every hover works normally. When a browser policy like this comes up again, confirm the desired tradeoff with the user rather than assuming "always show *something*" is preferred over "only ever the real thing." Note: the Claude Browser automation tool's own Chromium does NOT enforce this restriction (unmuted `.play()` succeeded even with `navigator.userActivation.hasBeenActive: false`), so this class of bug can't be reproduced or verified inside that tool — reason from documented browser autoplay-activation behavior instead of expecting a failing repro there.
- [2026-09-11] Related but distinct from the autoplay-activation lesson above: mobile browsers (iOS Safari especially) don't load ANY `<video>` data — not even the first frame needed to draw an idle poster — until the page has actually been interacted with, no matter what `preload` says. A canvas-keyed video idle frame (`videoWidth === 0` / `readyState < 2` until then) will stay blank on a first mobile visit until the visitor taps something. Fix: keep a plain fallback `<img>` (a still frame already extracted from the same clip) visible until the canvas has actually drawn a real frame, tracked with a boolean state flipped inside the draw function itself — this is a deterministic state check, not a browser-policy race, so unlike the autoplay lesson it doesn't need a real device to reason about correctly (though the Claude Browser tool's Chromium also doesn't reproduce the mobile no-preload restriction — it loaded video eagerly under mobile emulation, same as the autoplay case — so no failing repro was available there either).
- [2026-09-11] For a hero/banner video that needs real (non-muted) audio, don't fight the browser's autoplay-with-sound block — there's no JS workaround, full stop. Instead, design the audio-gating requirement into the UX itself: drop `autoplay`, let the video sit paused on its own first frame (loading it normally, since only playback needs a gesture, not loading), and put an explicit "Click to Start" prompt over it. The click that starts it is a real user gesture, so `.play()` there is unconditionally allowed — no fallback logic, no rejected-promise handling needed at all, unlike the hover-based cases above. Bonus: the same click also satisfies the separate mobile no-preload-without-interaction restriction (see the entry above) for free, since nothing is asked of the video until that click regardless. Keep header/footer/nav visible immediately and gate only the hero's own text/media behind the prompt — a public-facing informational site (especially a hospital's) shouldn't hide navigation or emergency contact info behind an interstitial for a cosmetic video.
- [2026-09-11] `navigator.userActivation.hasBeenActive` is scoped to the DOCUMENT, not to whichever component/page happens to be showing — it stays `true` across Next.js client-side route changes (no reload happens, so the browser never resets it), and only a genuine hard refresh/new navigation clears it back to `false`. This meant TASK 46's "Click to Start" gate reappearing on every in-app navigation back to the homepage was our own bug (local `useState` resetting on every fresh mount of the page), not a real browser limit — a mount-time check of `hasBeenActive` and auto-starting when it's already true fixed it, while a genuine refresh still correctly asks again (that part really is unavoidable). When debugging "does X need a fresh interaction" for anything gesture-gated, check whether the actual navigation is a full reload or a client-side route change — they are NOT equivalent for browser activation purposes.
- [2026-09-11] While debugging the above in the Claude Browser tool: `getComputedStyle(...).opacity` (and even a direct `element.style.opacity = '0'` write) can read back stale/unapplied values while the Browser pane is hidden/backgrounded — CSS transitions and style recalculation appear to be throttled for an unfocused tab, same as a real browser deprioritizes background tabs. A `computer{action:"screenshot"}` call forces the tab to paint and immediately resolved it. Don't trust a `getComputedStyle` read that contradicts the element's own className while the pane is reported as hidden — force a screenshot (or otherwise bring the tab forward) and recheck before concluding there's a real CSS bug.
